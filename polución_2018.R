...
library(readxl)
pol_2018 <- read_excel("C:/Users/Admin/Dropbox/bioestadística/tercer semestre/series de tiempo/trabajo_series/pol_2018.xlsx")
View(pol_2018)
dim(pol_2018)
names(pol_2018)
str(pol_2018)
...
#Serie NO diferenciada, estadística descriptiva#
hist(Temperature_C)
...
abline(h=15, col="red")
adf.test(serie_pol_24) #No requiere diferenciación por estacionariedad, pero aparentemente si por periodicidad#
ndiffs(serie_pol_24)
...
#Exploración de posible modelo, serie NO diferenciada#
...
#Exploración de posible modelo serie diferenciada#
...
autoplot(window(Train_pol_24_diff, start=0)) +
  ...
  autolayer(Train_autoARM_fit2, series="Naïve", PI=FALSE) +
  autolayer(Train_autoARM_fit3, series="Seasonal naïve", PI=FALSE) +
  xlab("Hours") + ylab("Temperature[°C]") +
  ggtitle("Temperatures in Bogotá-Suba, 2018") +
...
##TBATS##
tbats_model = tbats(Train_pol_24_diff)
tbats_forecast = forecast(tbats_model, h=length(valid_pol_24_diff))
MAPE(tbats_forecast$mean, valid_pol_24_diff)
plot(Train_pol_24_diff, col="blue", xlab="Hours", ylab="Temperature [°C]", main="Seasonal TBATS", type='l')
